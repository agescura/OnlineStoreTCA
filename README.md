...
2. Create an alert state inside of the reducer.
```swift
...
3. Extent `AlertState` and create as many alerts as you want. You can create a property wrapper or a function if you need some dynamic information.
...
3. Inside of the body of the reducer you can set the alert. As the state is an optional value, you need to implement `ifLet` in the reducer. This is a particular modifier that not need a reducer like a tipical `ifLet` reducer. 
...
4. Finally, implements `AlertView` modifier in the view. Remember, you use the binding `$` operator.
